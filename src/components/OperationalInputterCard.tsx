...
export function OperationalInputterCard({
  ...
  const save = async () => {
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Penginput gagal diperbarui.");
    }
  };

  const needsSetup = !name && !query.isLoading && !query.isError;
    ...
    <>
        ...
        <CardContent className="flex items-center justify-between gap-4 p-4">
          <div className="space-y-1">
            ...
            <p className="font-semibold">
              {query.isLoading ? "Memuat..." : query.isError ? "Gagal dimuat" : name || "Belum diatur"}
            </p>
            {needsSetup ? (
              <p className="text-sm text-muted-foreground">
                Nama penginput wajib diatur sebelum mencatat data.
              </p>
            ) : null}
          </div>
          <Button
          ...
          >
            ...
            {name ? "Ganti Penginput" : "Atur Penginput"}
          </Button>
        </CardContent>
      </Card>
      ...
      <Dialog open={open} onOpenChange={(next) => !mutation.isPending && setOpen(next)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{name ? "Ganti" : "Atur"} Penginput {label}</DialogTitle>
          </DialogHeader>
