...
export async function fetchActiveInterviewForm(outletId:string):Promise<InterviewFormVersion|null>{const {data,error}=await db.from("customer_interview_form_versions").select("id,outlet_id,version_number,is_active,customer_interview_questions(id,form_version_id,question_text,sort_order)").eq("outlet_id",outletId).eq("is_active",true).maybeSingle();if(error)throw error;if(!data)return null;return {...data,questions:[...(data.customer_interview_questions??[])].sort((a:InterviewQuestion,b:InterviewQuestion)=>a.sort_order-b.sort_order)};}
