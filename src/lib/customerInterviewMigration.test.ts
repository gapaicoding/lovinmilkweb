...
describe("parent interview migration",()=>{
 ...
 it("allows read-only RLS access for active Staff+",()=>{expect(sql.match(/for select to authenticated using\(public\.lm_is_active_staff_or_above\(\)\)/g)).toHaveLength(4);});
