...
-- PHASE 1 COMPATIBILITY: legacy RPCs remain functional during frontend rollout.
-- V3 RPCs require session IDs. A later cleanup migration may retire legacy
-- fallback only after the V3 frontend is deployed and production UAT passes.
begin;
...
comment on table public.operational_inputter_settings is 'Temporary Phase-1 compatibility source for legacy RPCs; V3 browser flows use operational_inputter_sessions.';
...
language plpgsql volatile security definer set search_path=public,pg_catalog as $$
declare v_session uuid; v_name text; v record;
begin
 ...
 if v_session is null then
   select btrim(s.inputter_name) into v_name from public.operational_inputter_settings s
   where s.outlet_id=public.lm_resolve_sales_outlet(p_outlet_id) and s.section=p_section;
   if v_name is null then raise exception 'Nama penginput belum diatur.' using errcode='P0001'; end if;
   return v_name;
 end if;
...
begin
 ...
 if v_session is null then new.inputter_name:=public.lm_get_active_operational_inputter(new.outlet_id,v_section);new.inputter_session_id:=null;return new;end if;
...
declare v_session uuid;v_outlet uuid;v record;begin v_outlet:=public.lm_resolve_sales_outlet(new.outlet_id);begin v_session:=coalesce(new.inputter_session_id,nullif(current_setting('app.operational_inputter_session_id',true),'')::uuid);exception when invalid_text_representation then v_session:=new.inputter_session_id;end;if v_session is null then new.outlet_id:=v_outlet;new.inputter_name:=public.lm_get_active_operational_inputter(v_outlet,'suppliers');new.inputter_session_id:=null;return new;end if;select * into v from public.lm_require_operational_inputter_session(v_session,'suppliers',v_outlet);new.outlet_id:=v.outlet_id;new.inputter_name:=v.inputter_name;new.inputter_session_id:=v.session_id;return new;end $$;
...
declare v_supplier_outlet uuid;v_requested_outlet uuid;v_session uuid;v record;begin select public.lm_resolve_sales_outlet(s.outlet_id) into v_supplier_outlet from public.suppliers s where s.id=new.supplier_id and s.deleted_at is null;if v_supplier_outlet is null then raise exception 'Supplier tidak valid untuk Item Supplier.' using errcode='23503';end if;if new.outlet_id is not null then v_requested_outlet:=public.lm_resolve_sales_outlet(new.outlet_id);if v_requested_outlet<>v_supplier_outlet then raise exception 'Outlet Item Supplier tidak sesuai dengan Supplier.' using errcode='23503';end if;end if;new.outlet_id:=v_supplier_outlet;begin v_session:=coalesce(new.inputter_session_id,nullif(current_setting('app.operational_inputter_session_id',true),'')::uuid);exception when invalid_text_representation then v_session:=new.inputter_session_id;end;if v_session is null then new.inputter_name:=public.lm_get_active_operational_inputter(v_supplier_outlet,'suppliers');new.inputter_session_id:=null;return new;end if;select * into v from public.lm_require_operational_inputter_session(v_session,'suppliers',v_supplier_outlet);new.inputter_name:=v.inputter_name;new.inputter_session_id:=v.session_id;return new;end $$;
...
declare v_session uuid;v_section text;v record;begin begin v_session:=coalesce(new.inputter_session_id,nullif(current_setting('app.operational_inputter_session_id',true),'')::uuid);exception when invalid_text_representation then v_session:=new.inputter_session_id;end;if v_session is null then new.inputter_name:=null;new.inputter_session_id:=null;return new;end if;v_section:=nullif(current_setting('app.operational_inputter_section',true),'');if v_section not in ('sales','visitors') then v_section:='visitors';end if;select * into v from public.lm_require_operational_inputter_session(v_session,v_section,new.outlet_id);new.inputter_name:=v.inputter_name;new.inputter_session_id:=v.session_id;new.outlet_id:=v.outlet_id;return new;end $$;
...
create or replace function public.create_operational_expense_v3(p_expense_date date,p_item_name text,p_quantity numeric,p_unit text,p_unit_price numeric,p_amount numeric,p_cost_category_id uuid,p_inputter_session_id uuid,p_receipt_reference text default null,p_vendor_name text default null,p_notes text default null)
returns public.operational_expenses language plpgsql volatile security definer set search_path=public,pg_catalog as $$ begin perform public.lm_require_operational_inputter_session(p_inputter_session_id,'expenses',(select outlet_id from public.cost_categories where id=p_cost_category_id));perform set_config('app.operational_inputter_session_id',p_inputter_session_id::text,true);return public.create_operational_expense(p_expense_date,p_item_name,p_quantity,p_unit,p_unit_price,p_amount,p_cost_category_id,p_receipt_reference,p_vendor_name,p_notes);end $$;
create or replace function public.save_supplier_with_items_v3(p_supplier jsonb,p_items jsonb,p_inputter_session_id uuid default null,p_supplier_id uuid default null,p_outlet_id uuid default null)
...
begin
 ...
 return public.save_supplier_with_items(p_supplier,p_items,p_supplier_id,p_outlet_id);
end $$;
create or replace function public.create_sales_transaction_with_visit_v3(p_transaction_date date,p_items jsonb,p_inputter_session_id uuid,p_notes text default null,p_entry_source text default 'manual',p_outlet_id uuid default null,p_existing_visit_id uuid default null,p_new_visit jsonb default null)
returns jsonb language plpgsql volatile security definer set search_path=public,pg_catalog as $$ begin perform public.lm_require_operational_inputter_session(p_inputter_session_id,'sales',p_outlet_id);perform set_config('app.operational_inputter_session_id',p_inputter_session_id::text,true);perform set_config('app.operational_inputter_section','sales',true);return public.create_sales_transaction_with_visit(p_transaction_date,p_items,p_notes,p_entry_source,p_outlet_id,p_existing_visit_id,p_new_visit);end $$;
create or replace function public.update_sales_transaction_with_visit_v3(p_transaction_id uuid,p_transaction_date date,p_items jsonb,p_notes text default null,p_existing_visit_id uuid default null,p_new_visit jsonb default null,p_inputter_session_id uuid default null)
...
begin
 if p_new_visit is not null then
   select outlet_id into v_outlet from public.sales_transactions where id=p_transaction_id and deleted_at is null;
   if v_outlet is null then raise exception 'Transaksi penjualan tidak ditemukan.' using errcode='P0002';end if;
   perform public.lm_require_operational_inputter_session(p_inputter_session_id,'sales',v_outlet);
   perform set_config('app.operational_inputter_session_id',p_inputter_session_id::text,true);
   perform set_config('app.operational_inputter_section','sales',true);
 end if;
 return public.update_sales_transaction_with_visit(p_transaction_id,p_transaction_date,p_items,p_notes,p_existing_visit_id,p_new_visit);
end $$;
create or replace function public.create_or_append_visitor_daily_recap_v3(p_business_date date,p_outlet_id uuid,p_inputter_session_id uuid,p_recorder_name text default null,p_entries jsonb default '[]')
returns jsonb language plpgsql volatile security definer set search_path=public,pg_catalog as $$ declare v record;v_header_name text;begin select * into v from public.lm_require_operational_inputter_session(p_inputter_session_id,'visitors',p_outlet_id);perform set_config('app.operational_inputter_session_id',p_inputter_session_id::text,true);perform set_config('app.operational_inputter_section','visitors',true);v_header_name:=case when exists(select 1 from public.visitor_daily_recaps r where r.outlet_id=v.outlet_id and r.business_date=p_business_date and r.deleted_at is null) then null else v.inputter_name end;return public.create_or_append_visitor_daily_recap(p_business_date,v.outlet_id,v_header_name,p_entries);end $$;
...
revoke all on function public.create_operational_expense_v3(date,text,numeric,text,numeric,numeric,uuid,uuid,text,text,text),public.save_supplier_with_items_v3(jsonb,jsonb,uuid,uuid,uuid),public.create_sales_transaction_with_visit_v3(date,jsonb,uuid,text,text,uuid,uuid,jsonb),public.update_sales_transaction_with_visit_v3(uuid,date,jsonb,text,uuid,jsonb,uuid),public.create_or_append_visitor_daily_recap_v3(date,uuid,uuid,text,jsonb) from public,anon,authenticated;
grant execute on function public.create_operational_expense_v3(date,text,numeric,text,numeric,numeric,uuid,uuid,text,text,text),public.save_supplier_with_items_v3(jsonb,jsonb,uuid,uuid,uuid),public.create_sales_transaction_with_visit_v3(date,jsonb,uuid,text,text,uuid,uuid,jsonb),public.update_sales_transaction_with_visit_v3(uuid,date,jsonb,text,uuid,jsonb,uuid),public.create_or_append_visitor_daily_recap_v3(date,uuid,uuid,text,jsonb) to authenticated;
