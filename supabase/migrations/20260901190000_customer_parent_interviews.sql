...
begin
 v_actor:=public.require_visitor_role(array['staff','admin','super_admin']); v_outlet:=public.lm_resolve_sales_outlet(p_outlet_id);
...
declare v_actor uuid;v_session record;v_version uuid;v_interview uuid;v_answer jsonb;v_question uuid;
begin
 ...
 select * into v_session from public.lm_require_operational_inputter_session(p_inputter_session_id,'interviews',p_outlet_id);
...
returns uuid language plpgsql volatile security definer set search_path=public,pg_catalog as $$
declare v_actor uuid;v_version uuid;v_answer jsonb;v_question uuid;
begin
 v_actor:=public.require_visitor_role(array['staff','admin','super_admin']);if p_interview_date is null or p_visit_time is null then raise exception 'Tanggal dan jam berkunjung wajib diisi.' using errcode='22023';end if;
 select form_version_id into v_version from public.customer_interviews where id=p_interview_id for update;if v_version is null then raise exception 'Wawancara tidak ditemukan.' using errcode='P0002';end if;
...
create policy interview_versions_read_staff on public.customer_interview_form_versions for select to authenticated using(public.lm_is_active_staff_or_above());
create policy interview_questions_read_staff on public.customer_interview_questions for select to authenticated using(public.lm_is_active_staff_or_above());
create policy interviews_read_staff on public.customer_interviews for select to authenticated using(public.lm_is_active_staff_or_above());
create policy interview_answers_read_staff on public.customer_interview_answers for select to authenticated using(public.lm_is_active_staff_or_above());
